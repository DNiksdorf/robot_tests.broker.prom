...
Подписание ЕЦП
    ...
    sleep  3
    Input Text    css=#PKeyPassword    199122
    sleep  3
    ...
    sleep  10
    reload page
    Wait Until Element Is Visible  css=.qa_with_ecp
...
Створити тендер
    [Arguments]   ${username}    ${tender_data}     ${plan_id}   ${token}
    log to console  ---------------------
...
Отримати інформацію із айтема
    [Arguments]   ${username}   ${tender_uaid}   ${field_name}
    click element  css=.
    ${return_value}=    Run Keyword If    ${field_name}' == 'milestones[0].code'   Get Text   xpath=(//span[contains(@class, 'qa_purchase_milestone_terms')])[1]
    ...  ELSE IF  '${field_name}' == 'milestones[0].title'       Get Text   xpath=(//span[contains(@class, 'qa_purchase_milestone_terms')])[1]
    ...  ELSE IF  '${field_name}' == 'milestones[0].percentage'      Get Text   xpath=(//span[contains(@class, 'qa_amount_of_payment')])[1]
    ...  ELSE IF  '${field_name}' == 'milestones[0].duration.days'       Get Text   xpath=(//span[contains(@class, 'qa_period_of_payment')])[1]
    ...  ELSE IF  '${field_name}' == 'milestones[0].duration.type'      Get Text   xpath=(//span[contains(@class, 'qa_period_of_payment')])[1]
    [Return]  ${return_value}
